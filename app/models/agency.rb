class Agency < ActiveRecord::Base
	...
	has_many	:treatments
  	...
  	def to_csv(agency)
  		CSV.generate do |csv|
  			csv << [agency.name]
			csv << ['Phase', 'Step', 'Activity', 'Question Number', 'Question', 'Response', 'Recorded At']
			agency.responses.order("phase_id ASC", "question_id ASC").each do |response|
				if response.question.name.present?
					question = response.question.name 
				else 
					question = response.question.content
				end
				
				csv << [response.phase_id, response.step_id, response.activity_id, response.question_id, 
					question, response.content, response.created_at
				]
			end
			...
			csv << ['Task', 'Concerns', 'Satisfaction Level', 'Due', 'Assigned', 'Details']
			agency.tasks.order("id ASC").each do |ts|
				csv << [ts.name, ts.concerns, ts.satisfaction_level, ts.due_at, ts.assigned_to, ts.content]
			end	
		end
  	end
	...
	def training_requirements_complete?( activity )
		self.responses.where( activity_id: activity.id ).count >= activity.questions.where( required: true ).count || 
		if self.responses.where( activity_id: activity.id ).first.present?
			self.responses.where( activity_id: activity.id ).first.content != 'yes' 
		end
	end
